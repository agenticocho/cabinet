...
[2026-04-13] Tasks board: clicking a conversation now opens a right-side detail panel instead of navigating away to the agent workspace. The panel shows a header with status icon, title, agent name, and time, plus an X close button. Running tasks display a live WebTerminal; completed/failed tasks show the ConversationResultView with prompt, summary, and artifacts. The board lanes shrink to 55% width when the panel is open.
