...
const MODELS_DIR =
  process.env.LLAMA_MODELS_DIR ?? "/Users/mikebird/models/llm/inference";
...
export const llamaLocalAdapter: AgentExecutionAdapter = {
  ...
  async testEnvironment() {
    ...
    try {
      execSync(`which ${binPath} 2>/dev/null || test -f ${binPath}`, {
        stdio: "ignore",
      });
    } catch {
      status = "fail";
      checks.push({
        ...
        hint: "Verify /opt/homebrew/bin/llama-server exists",
      });
    }
    return {
  ...
  async execute(
    ...
    const effortCtxMap: Record<string, number> = {
      low: 2048,
      medium: 8192,
      high: 32768,
    };
    ...
    const temperature =
      typeof (ctx.config as Record<string, unknown>)["temperature"] ===
      "number"
    ...
    const systemPrompt = [
      ...
      "",
      "At the END of your response, you MUST append a fenced memory block exactly, in this format:",
      "",
      "```memory",
