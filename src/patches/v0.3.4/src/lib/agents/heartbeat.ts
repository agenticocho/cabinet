...
export async function buildHeartbeatContext(slug: string, cabinetPath?: string): Promise<HeartbeatContext | null> {
  ...
  console.log('HB ctx persona', { slug, cabinetPath, found: !!persona, active: persona?.active, used: persona?.heartbeatsUsed, budget: persona?.budget, model: persona?.model });
...
Now execute your heartbeat. Check your focus areas, process inbox, review goals, and take action.`;
  ...
  const cwd =
    persona.workdir === "/data" || persona.workdir === "/"
      ? baseCwd
      : persona.workdir
...
export async function runHeartbeat(slug: string, cabinetPath?: string): Promise<string | null> {
  ...
  if ((persona.heartbeatsUsed ?? 0) >= persona.budget) {
    console.log(
  ...
  try {
    const meta = await startConversationRun({
      ...
      providerId: persona.provider,
      adapterType: "llama_local",
      adapterConfig: { model: persona.model },
      cabinetPath,
      cwd,
      timeoutSeconds: 600,
...
async function processHeartbeatOutput(
  ...
  const memoryMatch =
    output.match(/```memory\n([\s\S]*?)```/) ||
    output.match(/<memory>([\s\S]*?)<\/memory>/i);
  console.log("HB memoryMatch", slug, status, !!memoryMatch);
  console.log("HB output tail", slug, status, output.slice(-800));
  if (memoryMatch) {
  ...
  // Auto-pause after 3 consecutive failures (disabled for editor during Chilly bring-up)
  if (status === "failed" && slug !== "editor") {
...
/**
 * Run a heartbeat via the daemon adapter-session path.
 * All providers (including llama-local) go through startConversationRun →
 * createDaemonSession → daemon /sessions, so adapterType + adapterConfig.model
 * are always delivered to the adapter's AdapterExecutionContext.
 *
...
export async function startManualHeartbeat(
  ...
  const result = await runHeartbeat(slug, cabinetPath);
  console.log("MANUAL HB result", { slug, cabinetPath, result });
  return result;
}
