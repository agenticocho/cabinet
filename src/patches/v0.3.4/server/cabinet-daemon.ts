...
} from "../src/lib/agents/adapters/types";
import {
  getAppOrigin,
  getDaemonPort,
...
interface PtySession {
  ...
  ws: WebSocket | null;
  createdAt: Date;
  output: string[];   // <- change from string to string[]
  exited: boolean;
...
async function finalizeAdapterSessionConversation(input: {
  id: string;
  providerId: string;
  exitCode: number;
  output: string;
}): Promise<void> {
  const meta = await readConversationMeta(input.id);
  if (!meta) return;

  const plain = stripAnsi(input.output);
  const status =
    input.exitCode === 0 ? ("completed" as const) : ("failed" as const);

  completedOutput.set(input.id, {
    output: plain,
...
async function createAdapterSession(input: {
  sessionId: string;
  providerId?: string;
  adapterType?: string;
  ...
  const { sessionId, providerId, adapterType, adapterConfig, prompt, cwd, timeoutSeconds } = input;

  console.log("[daemon] createAdapterSession", sessionId, providerId, adapterType);

  const resolvedProviderId = resolveProviderId(providerId);
  const resolvedAdapterType =
    adapterType ??
    agentAdapterRegistry.defaultAdapterTypeForProvider(resolvedProviderId);
  const adapter = resolvedAdapterType
    ? agentAdapterRegistry.get(resolvedAdapterType)
    : undefined;

  if (!adapter) {
    throw new Error(
      `No adapter registered for provider ${resolvedProviderId}, adapterType ${adapterType ?? ""}`,
    );
  }

  if (!adapter.supportsDetachedRuns) {
    throw new Error(
      `Adapter ${adapter.type} for provider ${resolvedProviderId} does not support detached runs; must use PTY sessions.`,
    );
  }
    ...
    onLog: async (_stream, chunk) => {
      ...
      if (!session) {
        session = {
          ...
          autoExitFallbackTimer: undefined,
          claudeCompletionTimer: undefined,
          resolvedStatus: undefined,
          streamedText: "",
          structuredOutputBuffer: {} as any,
        };
        sessions.set(sessionId, session);
      }
      session.output.push(chunk);
      session.streamedText = (session.streamedText ?? "") + chunk;
  ...
  try {
    ...
    const streamedOutput =
      session && Array.isArray(session.output) && session.output.length > 0
        ? session.output.join("")
        : "";
    const fullOutput = streamedOutput || result.output || "";

    const exitCode =
      typeof result.exitCode === "number" ? result.exitCode : 0;

    console.log(
      "[daemon] createAdapterSession complete",
      sessionId,
      resolvedProviderId,
      resolvedAdapterType,
      {
        ...
        outputTail: fullOutput.slice(-300),
      },
    );

    completedOutput.set(sessionId, {
      output: stripAnsi(fullOutput),
      completedAt: Date.now(),
    });

    await finalizeAdapterSessionConversation({
      id: sessionId,
      providerId: resolvedProviderId,
      exitCode,
      output: fullOutput,
    ...
    console.error(
      "[daemon] createAdapterSession error",
      sessionId,
      resolvedProviderId,
      resolvedAdapterType,
      {
...
if (url.pathname === "/sessions" && req.method === "POST") {
  ...
  req.on("end", async () => {
    try {
      ...
      console.log("[daemon] adapter lookup", {
        providerId: resolvedProviderId,
        adapterType: resolvedAdapterType,
        hasAdapter: !!adapter,
        types: agentAdapterRegistry.list
          ? agentAdapterRegistry.list().map((a: any) => a.type)
          : "no-list",
      });

      if (prompt && adapter && adapter.supportsDetachedRuns) {
        console.log(
          "[daemon] POST /sessions using adapter path",
          sessionId,
          resolvedProviderId,
          resolvedAdapterType,
        );

        await createAdapterSession({
          sessionId,
          providerId: resolvedProviderId,
          adapterType: resolvedAdapterType,
        ...
        console.log(
          "[daemon] POST /sessions using PTY path",
          sessionId,
          resolvedProviderId,
          resolvedAdapterType ?? "none",
        );
